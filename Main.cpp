...
int main()
{
			...
			if (event.type == sf::Event::KeyPressed) //ili KeyReleased
			{
				...
				if (event.key.code == sf::Keyboard::Return)
				{
									...
									while (FRAME_DURATION <= lag)
									{
										...
										if (0 == game_won && 0 == pacman.get_dead())
										{
											...
											for (const std::array<Cell, MAP_HEIGHT>& column : map)
											{
												...
												// kaj ovo radi? doslovno gore imamo istu stvar napisanu
												/*if (0 == game_won)
												{
													break;
												}*/
					...
					{*/
						...
						////od jednom ne zeli radit, u principu mi ne smeta zapravo bolje mi je ovak al da
						////draw_text(336 - (strlen("Game Difficulty") * 8 / 2), 100 - strlen("Game Difficulty"), "Game Difficulty", Game_options);
	...
	//ghost_manager.reset(level, ghost_positions, gameSounds); 
	...
	//			pacman.update(level, map, gameSounds);

	//			ghost_manager.update(level, map, pacman, gameSounds);
	...
	//				myfile.open("Resources/High Score/High Score.txt");
	//				std::string txt_output;
	//				while (getline(myfile, txt_output)) {
	//					txt_output;
	//				}
	//				draw_text(CELL_SIZE * MAP_WIDTH - (text4.length() + 5) * 8, CELL_SIZE * MAP_HEIGHT, "High Score: " + txt_output, window);
	//				
